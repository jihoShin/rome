...
public class Atom10Parser extends BaseWireFeedParser {
	...
	private static final Namespace MEDIA_NS = Namespace.getNamespace(MEDIA_URI);
	...
	protected Namespace getMediaNamespace() {
		return MEDIA_NS;
	}
	...
	protected Entry parseEntry(final Feed feed, final Element eEntry, final String baseURI, final Locale locale) {
		...
		final Element youtube  = eEntry.getChild("group", getMediaNamespace());
		if(youtube != null){
			String mTitle = youtube.getChild("title", getMediaNamespace()).getText();
			String mContentUrl = youtube.getChild("content", getMediaNamespace()).getAttribute("url").getValue();
			String mThumbnailUrl = youtube.getChild("thumbnail", getMediaNamespace()).getAttribute("url").getValue();
			String mDescription = youtube.getChild("description", getMediaNamespace()).getText();
			final Media media = new Media();
			media.setTitle(mTitle);
			media.setContentUrl(mContentUrl);
			media.setThumbnailUrl(mThumbnailUrl);
			media.setDescription(mDescription);
